...
LIBNAME SASDATA BASE "O:\DSF\RTS\common\Pat\Permits\Salmon\Subsistence\2013";
RUN;
...
PROC SORT DATA = SASDATA.ISSUED2013; BY PERMIT;
PROC SORT DATA = SASDATA.PERSONAL; BY PERMIT;

DATA PERMITS; MERGE SASDATA.ISSUED2013 SASDATA.PERSONAL; BY PERMIT;
...
PROC FREQ DATA = PERMITS;
     TABLES STATUS VENDORCOPY RESPONDED FISHED;
RUN;
...
DATA REPORTED; MERGE PERMITS SASDATA.SALMON_HARVEST; BY PERMIT;
     ...
     KEEP PERMIT COMMUNITY R_RED R_KING R_COHO R_PINK R_CHUM;

PROC SORT DATA = REPORTED; BY COMMUNITY;
PROC MEANS SUM NOPRINT; BY COMMUNITY;
     VAR R_RED R_KING R_COHO R_PINK R_CHUM;
     OUTPUT OUT = REPORTED_BY_COMMUNITY SUM = R_RED R_KING R_COHO R_PINK R_CHUM; 
...
PROC MEANS SUM DATA = REPORTED_BY_COMMUNITY;
     VAR R_RED R_KING R_COHO R_PINK R_CHUM; 
     OUTPUT OUT = TOTAL_REPORTED SUM = TR_RED TR_KING TR_COHO TR_PINK TR_CHUM;
...
/*DATA EXPANDED; SET SASDATA.corrected_final_estimates; *NEEDED FOR 2005;*/
     IF VARIABLE = 'DAYS' OR VARIABLE = 'flounder' OR VARIABLE = 'TOTAL' THEN DELETE;
...
DATA EXPANDED; SET FLIPPED;
     ...
     E_PINK = PINK;  
     E_CHUM = CHUM; 
     KEEP E_RED E_KING E_COHO E_PINK E_CHUM;
RUN;
PROC PRINT;
RUN;
...
DATA FINAL; MERGE FINAL EXPANSION; BY _TYPE_;
     ...
     DROP _TYPE_ _FREQ_  E_RED E_KING E_COHO E_PINK E_CHUM EXPANSION_RED EXPANSION_KING EXPANSION_COHO EXPANSION_PINK EXPANSION_CHUM
          P_RESPONDED TR_RED TR_KING TR_COHO TR_PINK TR_CHUM;
RUN;
...
PROC MEANS SUM DATA = FINAL;
     VAR R_RED R_KING R_COHO R_PINK R_CHUM EXPANDED_RED EXPANDED_KING EXPANDED_COHO EXPANDED_PINK EXPANDED_CHUM;
RUN;
...
DATA EXPORT; 
     RETAIN COMMUNITY PERMITS RESPONDANTS NONRESPONDANTS P_NONRESPONSE PERMITS_FISHED R_RED R_KING R_COHO R_PINK R_CHUM
            EXPANDED_RED EXPANDED_KING EXPANDED_COHO EXPANDED_PINK EXPANDED_CHUM;
    SET FINAL;
    DROP MERGEVAR P;
RUN;
...
PROC EXPORT DATA= WORK.EXPORT
            OUTFILE= "O:\DSF\RTS\COMMON\Pat\Permits\Salmon\Subsistence\SPORT FISH PU PERMITS 2013" 
