...
DATA ISSUED_&RESPONSE_RATE; SET SASDATA.ISSUED_&RESPONSE_RATE; 

PROC SORT DATA = ISSUED_&RESPONSE_RATE; BY PERMIT;
PROC FREQ DATA = ISSUED_&RESPONSE_RATE;
...
PROC SORT DATA = HARVEST; BY PERMIT; RUN; 
LIBNAME SASDATA BASE "O:\DSF\RTS\common\PAT\PERMITS\&PROJECT\&YEAR\NONRESPONSE PROBLEM";


DATA ALLPERMITS; MERGE HARVEST (IN=INH) ISSUED_&RESPONSE_RATE (IN=INP); BY PERMIT;  
     IF INH THEN HARVFILE=1;
     IF INP THEN PERMFILE=1;
	 IF HARVFILE = . THEN HARVFILE = 0;
	 IF PERMFILE = . THEN PERMFILE = 0;
	 IF PERMFILE = 0 THEN DELETE;

RUN;

PROC PRINT DATA=ALLPERMITS;
     WHERE HARVFILE + PERMFILE NE 2;
...
DATA SASDATA.FISHED_MAILING2_&RESPONSE_RATE; SET FISHED_MAILING2;
...
DATA SASDATA.HARVESTED_&RESPONSE_RATE; SET HARVESTED;
RUN;



DATA HARVESTED; SET SASDATA.HARVESTED_&RESPONSE_RATE; 
...
DATA SASDATA.FINAL_ESTIMATES_&RESPONSE_RATE; SET BHH;
...
DATA BH_OUT; SET BH_;
      RENAME _1 = REPORTED_HARVEST_KENAI
             ...
             _5 = REPORTED_HARVEST_UNKNOWN
             _6 = REPORTED_HARVEST_TOTAL
...
DATA BHH_OUT; SET BHH;
      RENAME BHH1 = ESTIMATED_HARVEST_KENAI              SE1 = KENAI_HARVEST_SE
             ...
             BHH5 = ESTIMATED_HARVEST_UNKNOWN            SE5 = UNKNOWN_HARVEST_SE
             BHH6 = ESTIMATED_HARVEST_TOTAL              SE6 = TOTAL_HARVEST_SE;
RUN;
...
/*
PROC EXPORT DATA= WORK.RETURNED OUTFILE= "O:\DSF\RTS\common\PAT\PERMITS\Salmon\&YEAR\RETURNED_PERMITS.xls" DBMS=EXCELCS REPLACE;
RUN;
