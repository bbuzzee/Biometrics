...
DATA FIX_PERMITS; SET SASDATA.cislpm17c;
     RENAME MAILING = MAILING_P;
PROC SORT DATA = FIX_PERMITS; BY PERMITNO;
RUN;

DATA FIX_HARVEST; SET SASDATA.cislhv17c;
     MAILING_H = MAILING;
	 KEEP PERMITNO MAILING_H IMAGEPATH;
PROC SORT DATA = FIX_HARVEST; BY PERMITNO;
DATA FIX_HARVEST; SET FIX_HARVEST; BY PERMITNO;
     IF FIRST.PERMITNO;
RUN;

DATA FIX_MAILING; MERGE FIX_PERMITS FIX_HARVEST; BY PERMITNO;
     IF MAILING_H NE . AND RESPONSE_METHOD = 'mail';
     
PROC PRINT DATA = FIX_MAILING;
     WHERE MAILING_P NE MAILING_H;
	 VAR PERMITNO MAILING_P MAILING_H IMAGEPATH;
RUN;

PROC FREQ;
     WHERE MAILING_P NE MAILING_H;
     TABLES MAILING_P * MAILING_H / NOROW NOCOL NOPERCENT;
RUN;

PROC FREQ;
     TABLES MAILING_H / NOROW NOCOL NOPERCENT;
RUN;
...
DATA ISSUED; SET ISSUED;
     ...
     PERMIT = PERMITNO;
     IF OFFICE = '' OR COMMENTS = "DUMMY PERMIT" THEN VENDORCOPY = 'N';
        ELSE VENDORCOPY = 'Y';
     IF VENDORCOPY = 'N' THEN MAILING = 0;
...
PROC FREQ DATA = ISSUED;
     TABLES MAILING*VENDORCOPY / NOPRINT OUT = MAILING_BY_VENDOR;
PROC FREQ DATA = ISSUED;
