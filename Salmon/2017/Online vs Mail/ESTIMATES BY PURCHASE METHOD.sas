...
%LET GROUP = OO;  *OO, OM, VO, VM;
%LET GROUP2 = ONLINE;  * ONLINE, VENDOR;
...
TITLE1 "&PROJECT - &YEAR - PURCHASED &GROUP2";
...
DATA SASDATA.ISSUED; SET SASDATA.ISSUED;
    * IF GROUP = 'OO' OR GROUP = 'OM' OR GROUP = 'ON' THEN GROUP2 = 'ONLINE';
    * IF GROUP = 'VO' OR GROUP = 'VM' OR GROUP = 'VN' THEN GROUP2 = 'VENDOR';
     GROUP2 = GROUP;

PROC SORT DATA = SASDATA.ISSUED; BY GROUP2;
PROC FREQ DATA = SASDATA.ISSUED; BY GROUP2;
     TABLES MAILING / NOPRINT OUT = BN;

PROC TRANSPOSE DATA = BN OUT = BN; BY GROUP2;
      VAR COUNT;

DATA BN; SET BN (RENAME=(COL1 = BN_0 COL2 = BN_1 COL3 = BN_2));
     KEEP GROUP2 BN_0 BN_1 BN_2;
     IF GROUP2 = "&GROUP2";
RUN;
PROC PRINT DATA = BN;
RUN;
...
DATA SASDATA.FISHED_MAILING2_&GROUP2; SET FISHED_MAILING2;
...
DATA SASDATA.HARVESTED_&GROUP2; SET HARVESTED;
RUN;



DATA HARVESTED; SET SASDATA.HARVESTED_&GROUP2; 
...
PROC FREQ DATA = SASDATA.ISSUED;
     TABLES ISSUE_METHOD / OUT = ISSUED_GROUP NOPRINT;

DATA ISSUED_ONLINE; SET ISSUED_GROUP;
     IF ISSUE_METHOD = 'ONLINE';
     GROUP2 = ISSUE_METHOD;
     RENAME COUNT = NHAT;
     DROP PERCENT ISSUE_METHOD;
	 MERGEVAR = 1;

DATA ISSUED_VENDOR; SET SASDATA.TOTAL_ISSUED;
     RENAME NHAT = TOTAL_ISSUED;
	 MERGEVAR = 1;
DATA ISSUED_VENDOR; MERGE ISSUED_VENDOR ISSUED_ONLINE; BY MERGEVAR;
     NHAT = TOTAL_ISSUED - NHAT;
	 GROUP2 = 'VENDOR';

DATA TOTAL_ISSUED; SET ISSUED_ONLINE ISSUED_VENDOR;
	 IF VAR_NHAT = . THEN VAR_NHAT = 0;
     DROP TOTAL_ISSUED MERGEVAR;
...
DATA BNH_D; MERGE TOTAL_ISSUED (KEEP=NHAT VAR_NHAT) BN FISHED_MAILING2;
...
DATA BH_X; SET BH_C(IN=C DROP=BN_C RENAME=(_1=BHH1 _2=BHH2 _3=BHH3 _4=BHH4 _5=BHH5 _6=BHH6)) 
                LHB_D(IN=DR DROP= _TYPE_ LN_D VAR_MEAN1-VAR_MEAN&MATRIX SE1-SE&MATRIX RENAME=(_1=BHH1 _2=BHH2 _3=BHH3 _4=BHH4 _5=BHH5 _6=BHH6)) 
