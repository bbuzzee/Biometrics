...
DATA ISSUED; SET SASDATA.&DATA;
     IF MAILING = 9 THEN RM = 'NEITHER';
     IF MAILING NE 9 THEN RM = UPCASE(RESPONSE_METHOD);
     DROP RESPONSE_METHOD;

DATA ISSUED; SET ISSUED;
     CITY = UPCASE(CITY);
     RESPONSE_METHOD = RM;
 
     IF PERMITNO GE 60000 THEN ISSUE_METHOD = 'ONLINE  ';
	    ELSE ISSUE_METHOD = 'VENDOR  ';

PROC FREQ;
     TABLES ISSUE_METHOD * RESPONSE_METHOD;
RUN;

DATA ISSUED; SET ISSUED;
     IF ISSUE_METHOD = 'ONLINE' AND RESPONSE_METHOD = 'ONLINE' THEN GROUP = 'OO';
     IF ISSUE_METHOD = 'ONLINE' AND RESPONSE_METHOD = 'MAIL' THEN GROUP = 'OM';
     IF ISSUE_METHOD = 'ONLINE' AND RESPONSE_METHOD = 'NEITHER' THEN GROUP = 'ON';
     IF ISSUE_METHOD = 'VENDOR' AND RESPONSE_METHOD = 'ONLINE' THEN GROUP = 'VO';
     IF ISSUE_METHOD = 'VENDOR' AND RESPONSE_METHOD = 'MAIL' THEN GROUP = 'VM';
     IF ISSUE_METHOD = 'VENDOR' AND RESPONSE_METHOD = 'NEITHER' THEN GROUP = 'VN';

     RENAME STATUS = S;

PROC FREQ DATA = ISSUED; 
     TABLES GROUP*MAILING;
RUN;
...
PROC SORT DATA = ISSUED; BY GROUP PERMIT;
...
PROC FREQ DATA = ISSUED; 
     TABLES STATUS * RESPONDED / OUT = SUMMARY_RESPONSE_GROUP;
RUN;

PROC FREQ DATA = ISSUED; 
     TABLES GROUP * STATUS * RESPONDED / OUT = SUMMARY_RESPONSE_GROUP;
RUN;
PROC PRINT DATA = SUMMARY_RESPONSE_GROUP;
RUN;
...
PROC PRINT DATA = ISSUED;
     WHERE STATE NE 'AK';
     VAR PERMIT FAMILYSI CITY STATE STATUS LICENSE_NO ISSUE_METHOD OFFICE;
     TITLE2 'NON RESIDENT PROBLEMS';
...
DATA PERSONAL; SET ISSUED;
     KEEP PERMIT GROUP ADLNO CITY STATE FIRST_NAME LAST_NAME;

DATA ISSUED; SET ISSUED;
     DROP DUPPREF ADLNO CITY STATE FIRST_NAME LAST_NAME HHMEMBERS LICENSE_NO;
