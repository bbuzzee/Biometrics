...
*  This program is stored as O:\DSF\RTS\pat\permits\salmon\2007\estimates.sas          *;
****************************************************************************************;

%LET SCENARIO = SCENARIO_7;
%LET PROJECT = SALMON;
%LET PROGRAM = ESTIMATES;
%LET DATA = harvest2011;
%LET YEAR = 2011;
%LET SPECIES = SALMON;
%LET SPECIES_COMMA = RED,KING,COHO,PINK,CHUM,FLOUNDER; 
%LET SPECIES_LIST = RED KING COHO PINK CHUM FLOUNDER; 

%LET GILLNET_START = '15JUN11'D; 
%LET GILLNET_STOP = '24JUN11'D; 
...
PROC SORT DATA=SASDATA.SALMON_HARVEST_&SCENARIO; BY PERMIT; RUN;  
PROC SORT DATA=SASDATA.ISSUED; BY PERMIT; RUN;

DATA ALLPERMITS; MERGE SASDATA.SALMON_HARVEST_&SCENARIO (IN=INH) SASDATA.ISSUED (IN=INP); BY PERMIT;  
...
DATA FISHED_MAILING2; SET SASDATA.FISHED_MAILING2; 
...
DATA SASDATA.FINAL_ESTIMATES_&SCENARIO; SET BHH;
RUN;
...
DATA BHH_OUT; SET BHH_OUT;
     DROP ESTIMATED_HARVEST_UNK_FIXABLE 
          KENAI_HARVEST_SE KASILOF_DIPNET_HARVEST_SE KASILOF_GILLNET_HARVEST_SE FISH_CREEK_HARVEST_SE UNKNOWN_HARVEST_SE
          UNK_FIXABLE_HARVEST_SE TOTAL_HARVEST_SE;


PROC EXPORT DATA= WORK.BHH_OUT
            OUTFILE= "O:\DSF\RTS\PAT\PERMITS\&PROJECT\&YEAR\PU PERMITS BOF SCENARIOS.XLS" 
            DBMS=EXCEL2000 REPLACE;
            SHEET="&SCENARIO"; 
RUN;
