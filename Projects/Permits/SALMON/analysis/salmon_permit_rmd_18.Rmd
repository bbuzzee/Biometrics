...
# write.csv(master_df, "../data/master_df_18.csv")
...
reported <- master_df %>% group_by(city, fishery) %>% summarise(red = sum(red),
                                                       king = sum(king),
                                                       coho = sum(coho),
                                                       pink = sum(pink), 
                                                       chum = sum(chum),
...
region_df <- left_join(subsistence_master, regions_key, by = "city") %>% select(7, 14:18, 20)



total_salmon_region <- region_df %>% group_by(region_number, fishery) %>% summarize_all(sum) %>% mutate(total_salmon = exp_reds + exp_kings + exp_coho + exp_pink + exp_chum) %>% select(1,2,8)
