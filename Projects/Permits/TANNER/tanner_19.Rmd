...
```{r}
total <- harvest %>% group_by(areacode) %>% summarize(crab_count = sum(harvestcount, na.rm = T), days = sum(effort_days))
...
```{r}
knitr::kable(cbind(areacode = c("ABC", "D", "E"), crab_count = round(172*proportions$prop_non_rspd_fished*mean_catches$mean, 0), days = round(172*proportions$prop_non_rspd_fished, 0)))
```
...
SE_catch <- sqrt(N^2*(prop_non_rspd_fished^2*var_xhat$`var(harvestcount, na.rm = T)` + mean_catches$mean^2*var_phat))


SE_days <- sqrt(N^2*var_phat)
se_df <- data.frame(cbind(areacode = c("ABC", "D", "E"), se_counts = round(SE_catch, 0)), se_days = round(SE_days, 1))
