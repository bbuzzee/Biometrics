...
First we will initialize our "population" and assign each fish a location in a simplified square lake.
...
# MC Simulation
Finally, lets repeat the process 1000 times for a variety of different capture probabilities and population sizes.
...
results %>% group_by(true_N, region1_prob, region2_prob) %>% summarize(chisq_power = sum(reject_equal_p)/n(),
                                                               avg_bias_raw = mean(bias_raw),
                                                               avg_bias_stratified = mean(bias_stratified)
                                                               ) %>% knitr::kable()
